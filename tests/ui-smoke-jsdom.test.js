...
function startDevServer() {
  ...
  let stdout = '';
  const onData = (d) => { stdout += d.toString(); };
...
async function assertRootAndBlog(baseUrl) {
  ...
  const activeNav = Array.from(docRoot.querySelectorAll('header a')).find((a) => (a.className || '').includes('text-primary-600'));
  if (!activeNav) throw new Error('No active nav item (text-primary-600) found on root HTML');
  if (activeNav.getAttribute('href') !== '/') throw new Error('Active nav item on root does not point to /');
  ...
  if (!footerLink) throw new Error('Footer developer credit link not found in root HTML');
  ...
  if (!blogClasses.includes('text-primary-600')) throw new Error('Blog link not active on /blog (missing text-primary-600 class)');
}
